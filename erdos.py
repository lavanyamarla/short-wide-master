...
def create_graph(dim, p):
	random_graph = nx.erdos_renyi_graph(dim, p)
	weights = {}

	for edge in random_graph.edges():
		weights[edge] = np.random.power(2.76)

	nx.set_edge_attributes(random_graph, weights, 'weight')

	random_graph = nx.adjacency_matrix(random_graph)
...
if __name__ == "__main__":
	...
	for i in range (100):
		...
		if (i + 1) % 10 == 0:
			with open('raw_output/erdos/' + str(p) + '_weighted.pkl', 'wb') as f:
				pickle.dump(weight, f)

			with open('raw_output/erdos/' + str(p) + '_geodesic.pkl', 'wb') as f:
				pickle.dump(geo, f)

			with open('raw_output/erdos/' + str(p) + '_sw_d.pkl', 'wb') as f:
				pickle.dump(dijk, f)
	...
	with open('raw_output/erdos/' + str(p) + '_sw_d.pkl', 'wb') as f:
        ...
        s_w = float(sys.argv[3])
        s_g = float(sys.argv[4])
        s_s = float(sys.argv[5])
